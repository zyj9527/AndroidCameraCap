...
int CameraIn::openDev(char *path)
{
    ...
    if (mFormat.fmt.pix.pixelformat == v4l2_fourcc('Y', 'V', '1', '2'))
    {
        ...
        printf("Special handling for YV12 on Stride %d, size %d", mFormat.fmt.pix.bytesperline, mFormat.fmt.pix.sizeimage);
    }
...
int CameraIn::setupBuffer()
{
	...
	for (int i = 0; i < NO_OF_CAPTURE_BUF; i++)
    {
		...
		mCapBuf[i].start = (unsigned char*)mmap(NULL, mCapBuf[i].length, PROT_READ | PROT_WRITE, MAP_SHARED, mFd, mCapBuf[i].offset);
